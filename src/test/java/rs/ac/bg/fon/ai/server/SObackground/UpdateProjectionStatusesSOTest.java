...
class UpdateProjectionStatusesSOTest {
    ...
    @BeforeEach void setUp() { so = new UpdateProjectionStatusesSO(); }
    @AfterEach void tearDown() { so = null; }
    @Test void testUpdateProjectionStatusesSO() { assertNotNull(so); }
    @Test void testPrecondition() { assertDoesNotThrow(() -> so.precondition(null)); }
    @Test void testGetResultPreIzvrsavanja() { assertEquals(0, so.getResult()); }
}
